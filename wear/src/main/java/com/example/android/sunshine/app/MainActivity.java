...
import static com.example.android.sunshine.app.WearableListenerService.LOGD;
...
/**
 * Created by lecoq on 06/08/2016.
 */
    ...
    public void onDataChanged(DataEventBuffer dataEvents) {
        ...
        for (DataEvent event : dataEvents) {
            if (event.getType() == DataEvent.TYPE_CHANGED) {
                ...
                if (WearableListenerService.IMAGE_PATH.equals(path)) {
                    DataMapItem dataMapItem = DataMapItem.fromDataItem(event.getDataItem());
                    Asset photoAsset = dataMapItem.getDataMap()
                            .getAsset(WearableListenerService.IMAGE_KEY);
                    // Loads image on background thread.
                    new LoadBitmapAsyncTask().execute(photoAsset);

                } else if (WearableListenerService.COUNT_PATH.equals(path)) {
